...
//
// Phase 1 (GROWTH-PHASES.md): these were pure-SEO programmatic pages. Under a
// social-only model we keep ONLY topics in live categories (those tied to a real
// cluster/offer/content) — as internal leak surfaces, not for ranking. The rest are
// filtered out of the build below. The raw list is retained for reference/restore.
import { LIVE_CATEGORIES } from './taxonomy';
...
// Live topics only — junk SEO categories (b2b, gaming, education, etc.) are dropped.
export const topics: Topic[] = rawTopics.filter(t => LIVE_CATEGORIES.has(t.category));
